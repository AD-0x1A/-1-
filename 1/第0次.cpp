...
customer * read_customer_txt() //读取customer。txt内容 
{
	...
	int i=0;
	char b;
	fp = fopen("C:\\Users\\Cainge\\Desktop\\第1次\\customer.txt","r"); 
	if(NULL==fp)
	{
...
orders * read_orders_txt()//读取orders.txt内容 
{
	...
	long d;
	FILE *fp;
	fp = fopen("C:\\Users\\Cainge\\Desktop\\第1次\\orders.txt","r");
	if(fp == NULL)
	{
...
lineitem * read_lineitem_txt()//读取lineitem.txt内容
{
	...
	int i=0;
	char b,c;
	fp = fopen("C:\\Users\\Cainge\\Desktop\\第1次\\lineitem.txt","r");
	if(fp==NULL)
	{
...
select_result * Select(customer * cus,orders * ord,lineitem * item,char * order_date,char * ship_date,char * mktsegment)//进行选择 
{
	...
	{
		//printf("%d\n",i);
		if(i==0&&result2[m].l_extendedprice==0)
		{
